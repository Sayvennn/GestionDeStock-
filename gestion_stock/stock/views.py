...
def liste_operations(request):
    ...
    if statut_filtre != "all":
        entrees_qs = entrees_qs.filter(statut=statut_filtre).filter(employe=request.user)
        sorties_qs = sorties_qs.filter(statut=statut_filtre).filter(employe=request.user)
